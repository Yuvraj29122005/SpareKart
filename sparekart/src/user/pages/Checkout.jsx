...
function Checkout({ cart, cartCount, addOrder }) {
    ...
    const placeOrder = async () => {
        ...
        try {
            const paymentType = form.payment === "COD" ? "Cash on Delivery" : "Online Payment";

            if (paymentType === "Online Payment") {
                const res = await loadRazorpayScript();
                if (!res) { alert("Razorpay SDK failed to load."); return; }

                const data = await apiFetch("/orders/razorpay-create", {
                    method: "POST",
                    body: JSON.stringify({ directItems: buyNowItems || null })
                });
                ...
                const options = {
                    ...
                    handler: async function (response) {
                        try {
                            ...
                            alert(err.message || "Failed to confirm order after payment");
                        }
                    },
                    prefill: {
                        name: form.name,
